 ...
 *
 */
public class HotspotLongGenerator extends NumberGenerator {
  ...
  public Long nextValue() {
	...
	/**
	 * Since Random.nextLong() generates random long which can be negative,
	 * we shift the bits to the left and back by 1 bit to introduce the MSB as zero 
	 */
        
    if (random.nextDouble() < hotOpnFraction) {
      // Choose a value from the hot set.
      value = lowerBound + (((Utils.random().nextLong() << 1) >>> 1) % hotInterval)  ;
      
    } else {
      // Choose a value from the cold set.
      value = lowerBound + (((Utils.random().nextLong() << 1) >>> 1) % coldInterval)  ;
    }
    setLastValue(value);
    return value;
  }
