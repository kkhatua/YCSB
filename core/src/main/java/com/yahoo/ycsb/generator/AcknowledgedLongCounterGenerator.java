...
/**
 * A CounterGenerator that reports generated Long via lastLong()
 * only after they have been acknowledged.
 */
public class AcknowledgedLongCounterGenerator extends LongCounterGenerator
{
	 ...
	 */
	public AcknowledgedLongCounterGenerator(long countstart)
	{
		super(countstart);
	...
	{
		//TODO: Verify that Int casts are correct
		...
		if (lock.tryLock()) {
			...
			try {
			  ...
			  int beforeFirstSlot = (int) (limit & WINDOW_MASK);
				int index;
				for (index = (int) (limit + 1); index != beforeFirstSlot; ++index) {
